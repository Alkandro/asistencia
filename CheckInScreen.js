...
const CheckInScreen = () => {
  ...
  // ==== Obtener datos de usuario (cinturón, total entrenos, nombre) ====
  const fetchUserData = useCallback(async () => {
    try {
      const user = auth.currentUser;
      if (!user) return;
  ...
  const getBeltColor = (belt) => {
    const beltColorMap = {
      white: "#000000", // Texto negro para cinturón blanco
      blue: "#4285F4",
      purple: "#AA60C8",
      brown: "#8B4513",
      black: "#000000",
  ...
  const ListHeader = () => (
    <View style={styles.headerContainer}>
      ...
      {/* Mensajes */}
      {latestMessage ? (
        <CardMinimal style={styles.messageCard}>
          <View style={styles.messageHeader}>
            <Text style={styles.messageHeaderTitle}>{t("Mensajes")}</Text>
          </View>
        ...
        <CardMinimal style={styles.messageCard}>
          <Text style={styles.noMessageText}>{t("No hay mensajes aún")}</Text>
        </CardMinimal>
      )}
    </View>
  );
    ...
    return (
      <View style={styles.footerContainer}>
            ...
            <View style={styles.progressCircle}>
              <View 
                style={[
                  styles.progressFill,
                  { 
                    height: `${(countInGroup / groupSize) * 100}%`,
        ...
        <Text style={styles.monthlyStats}>
          {t("You have worked out {{count}} times this month", { count: monthlyCheckIns })}
        </Text>

        {lastRatingDate && (
          <Text style={styles.lastRatingText}>
            {t("Ultima puntuación:")}{" "}
            {lastRatingDate
              ? dayjs(lastRatingDate.toDate()).format("DD/MM/YYYY HH:mm")
              : ""}

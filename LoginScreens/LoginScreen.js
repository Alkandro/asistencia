...
import { doc, updateDoc, getDoc } from "firebase/firestore"; // 🆕 IMPORTS AGREGADOS
...
const LoginScreen = ({ navigation }) => {
  ...
  const updateUserActivity = async (userId) => {
    try {
      console.log('🔄 Actualizando lastActivity para usuario:', userId);
      ...
      if (userDoc.exists()) {
        await updateDoc(userRef, {
          lastActivity: now,
          lastLogin: now, // También guardamos cuándo fue el último login
        });
        console.log('✅ lastActivity actualizado exitosamente');
      } else {
        console.warn('⚠️ Documento de usuario no encontrado:', userId);
      }
    } catch (error) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      console.log('✅ Login exitoso para usuario:', user.uid);
      
      // 🆕 ACTUALIZAR ÚLTIMA ACTIVIDAD DESPUÉS DEL LOGIN EXITOSO
      ...
      switch (error.code) {
        ...
        case "auth/invalid-credential":
          errorMessage = t("Credenciales inválidas");
          break;
        default:
